...
初步对时后也可能出现**连续缓慢漂移**而非一次突跳。逐条就近配对会把重复出现的
事件码接错，从而掩盖真实对时关系。本系统是一个 React + FastAPI 全栈复核台：
工程师在页面上编辑（或文本导入）两条事件流，设定允许偏移范围、固定跳变量
（或启用**缓变校时**并填写相邻偏移变化上限）与最低命中数，经真实业务 API
发起裁决，并逐对核对时间、事件码以及各对实际偏移的变化。
...
### 缓变校时模式（可选，`drift_mode=true`）

初步对时后两卡时钟可能在连续命中间缓慢漂移，固定偏移与单次跳变模型会把
真实同码事件拒之门外。启用后**不再使用固定跳变量**，仍沿用两条事件流、
偏移范围与最低命中数，可行方案为满足以下条件的配对序列：

1. 两侧事件码相同，两侧索引严格递增；
2. 每对偏移 `tA_i − tB_j` 位于 `[offset_min, offset_max]` 内；
3. 每相邻两对的偏移变化（整数之差）绝对值不超过 `max_drift`。

裁决目标：

- **先最大化匹配数**；不足最低命中数判无解。
- 同分时按 **初始偏移 `d0`（首对偏移）升序、配对索引序列字典序** 取规范首解；
  歧义时附带规范序下的第二份见证。
- 响应逐对给出**实际偏移**及**相对上一对的变化**，供工程师判定漂移是否连续。

> 算法依据：可行配对即 DAG 节点（边 = 索引递增且偏移差在限内），最长链即
> 最优；固定 B 索引时配对偏移随 A 索引严格递增，每列可按偏移二分窗口，
> 反向 DP 在 O(配对数 × m) 内完成；见证利用“分叉越晚候选越优”自后向前
> 找首个可行分叉点（见 `backend/app/engine.py` 顶部说明）。

## 目录结构

```
backend/            FastAPI 裁决服务
  app/engine.py     裁决引擎（静态/跳变 + 缓变校时：规范序、唯一/歧义、见证）
  app/schemas.py    Pydantic 输入校验与响应契约
  app/main.py       /health、/api/meta、/api/adjudicate
  tests/            844 项测试（含对暴力枚举的 1000 组随机差分测试）
...
1. 后端 `pytest`（引擎手工用例 + 静态/缓变两组共 1000 组随机差分测试 + API 契约测试）；
2. 前端依赖安装与 `node --test` 单元测试；
3. 前端生产构建检查（`vite build`）；
4. 等待 `GET /health` 通过；
5. HTTP 冒烟：对**真实运行中的 API** 发起裁决，断言跳变对时、逐对偏移、
   未达最低命中的无解、422 校验、歧义见证等契约，并覆盖缓变校时新模式
   （连续漂移对时、相邻变化超限断裂、歧义见证、参数失败边界）与静态模式
   兼容性（省略 `drift_mode` 与显式 `false` 响应逐字节一致）。
...
缓变校时模式下响应另含 `mode: "drift"` 与 `drift_solution`（歧义时附
`drift_witness`）：每对给出实际偏移 `offset` 与相对上一对的变化
`offset_change`（首对为 `null`），供判定漂移是否连续；静态/跳变模式的
请求与响应字段保持原样，不携带这些新增键。
